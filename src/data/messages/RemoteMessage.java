 ...
 * methods, it is expected that the extending class will define message types as
 * static constants (though technically not required).
 * 
 * @author Andrew Alm
 */
public class RemoteMessage extends Message {
    ...
    /**
     * Adds a parameter to the parameter queue, when the message is received
     * parameters can be dequeued in the same order they were put on by the
     * sender.
     * 
     * @param parameter
    ...
    /**
     * Retrieves the first parameter queued returning it. If the parameter queue
