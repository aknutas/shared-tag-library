...
public abstract class RemoteObject implements ClientResponder {
	...
	public RemoteMessage block(long timeout) {
		...
		if (0 == timeout)
		    ...
		    acquired = this.lock.tryAcquire(timeout,
    ...
    /**
     * Creates a new RemoteObject from the given connection and network Control
     * object.
     * 
     * @param connection
     *            the connection id
     ...
     *             if the network control object is null
     * @throws RemoteObjectException
     *             if the RemoteObject cannot be created (timeout = 5000 ms)
     */
    public RemoteObject(int connection, Control network)
    ...
    /**
     * Creates a new RemoteObject from the given connection and network Control
     * object. In order for a remote object to be created, the connection must
     * respond to a ping.
     * 
     * @param connection
     *            the connection id
     ...
     *             if the network control object is null
     * @throws RemoteObjectException
     *             if the RemoteObject cannot be created
     */
    public RemoteObject(int connection, Control network, long timeout)
	    throws NullPointerException, RemoteObjectException {
	if (null == network)
    ...
    /**
     * Sends the given message over the connection this RemoteObject contains,
     * returning the response message. This method will block until there is a
     * response.
     * 
     * @param message
     *            the message to send
     ...
     */
    public RemoteMessage send(RemoteMessage message)
	    throws NullPointerException {
     ...
     *            the message to send
     * @param timeout
     *            the maximum amount of time to wait for a response, or 0 to
     *            wait indefinitely
     * 
     * @return the response message, or null if the operation timed out
     * 
     * @throws NullPointerException
     *             if the message given is null
     */
    public RemoteMessage send(RemoteMessage message, long timeout)
	    throws NullPointerException {
	if (null == message)
    ...
    /**
     * Pings the remote connection to determine if the connection is still
     * alive.
     * 
	...
	this.network.sendLibraryMsg(this.connection, new RemoteMessage(
		RemoteMessage.MSG_PING), response);
	...
	if (RemoteMessage.MSG_PING == ((RemoteMessage) message)
		.getMessageType())
	    return;
    }

    /**
     * This method does nothing in this class, however may be overridden by
     * extending classes for more advanced behavior, if needed.
     * 
     * @param connection
     *            the connection id
     */
