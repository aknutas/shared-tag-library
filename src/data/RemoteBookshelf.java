...
/**
 * A RemoteBookshelf extends the RemoteObject class and implements the Bookshelf
 * interface. This class accessing a Bookshelf on a remote system using the
 * Bookshelf interface.
 * 
 * @author Andrew Alm
 */
public class RemoteBookshelf extends RemoteObject implements Bookshelf {
    ...
    /**
     * Creates a new RemoteBookshelf object with the given network, connection,
     * and id.
     * 
     * @param network
     ...
     *            the connection id
     * @param id
     *            the id of the the RemoteBookshelf (given by server)
     * 
     * @throws NullPointerException
     *             if the network given is null.
     * @throws RemoteObjectException
     *             if the RemoteBookshelf cannot be created.
     */
    ...
    /**
     * Determines whether the RemoteBookshelf object is empty. An empty
     * RemoteBookshelf has a size of zero.
     * 
    ...
    /**
     * Gets the size of a RemoteBookshel. The size is the number of books in the
     * shelf (this may be greater than the number of books an iterator returns).
     * If there is an error this method will return 0, although 0 may indicate
     * an empty bookshelf.
     * 
    ...
    @Override
    public int size() {
	RemoteMessage response = this.send(new BookshelfMessage(
		BookshelfMessage.MSG_SIZE, this.id));
	if (null == response)
	    return 0;
    ...
    /**
     * Gets the value of the property of the given name, or null if the property
     * name is not found (or there is another error).
     * 
     * @param name
	...
	/* send message */
	BookshelfMessage message = new BookshelfMessage(
		BookshelfMessage.MSG_GET, this.id);
    ...
    /**
     * This method is used to set a property of the given name to the given
	...
	/* send message */
	BookshelfMessage message = new BookshelfMessage(
		BookshelfMessage.MSG_SET, this.id);
	message.queueParameter(name);
    ...
    /**
     * Gets an iterator containing all of the properties of the RemoteBookshelf
     * object. If there is an error getting the iterator then null is returned.
     * 
     * @return an Iterator of String-String pairings, or null on error
     */
    @Override
    public Iterable<Entry<String, String>> enumerateProperties() {
	BookshelfMessage message = new BookshelfMessage(
		BookshelfMessage.MSG_PROPERTY_ITERATOR, this.id);
	RemoteMessage response = this.send(message, 5000);
	if (null == response)
	    return null;
	...
	return new Iterable<Entry<String, String>>() {
		...
		try {
		    return new RemotePropertyIterator(network, connection,
			    iteratorId);
    ...
    /**
     * Gets an iterator of books on the RemoteBookshelf. If there is an error
     * getting the iterator then null is returned.
     * 
	...
	try {
	    RemoteMessage response = this.send(new BookshelfMessage(
		    BookshelfMessage.MSG_ITERATOR, this.id), 5000);
	    ...
	    return new RemoteBookIterator(this.network, this.connection, id
		    .intValue());
	} catch (RemoteObjectException ex) {
	    return null;
	}
    ...
    /**
     * Creates a bookshelf containing a subset based on the comparable object
     * given.
     * 
     * @param comparable
     *            the Comparable to use to determine whether a book is in a
     *            subset
     * 
