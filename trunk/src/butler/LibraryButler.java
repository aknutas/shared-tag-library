...
/**
 * The LibraryButler abstract class is an implementation of
 * LibraryButlerInterface. It contains a Self-Organizing Map that is trained
 * with a Collection of VirtualBookshelfs to classify books into these shelfs.
 * 
 * @author sjpurol
 * 
 */
public abstract class LibraryButler implements LibraryButlerInterface {
    ...
    /**
     * Contains pairs of Strings and Integers mapping each tag to a specific
     * input neuron.
     */
	...
	for (Map.Entry<String, Integer> tag : b.enumerateTags()) {
		...
		inputValues[current] += ((double) tag.getValue());// /
		// maxTagMag);
		// inputValues[current.getValue()] += ( (double)tag.getValue() /
		// maxTagMag);
	    } else
		inputValues[current] = ((double) tag.getValue());// /
	    // maxTagMag);
    ...
    /**
     * Typically called by the HeadButler. Runs the input book through the brain
     * and returns an entry containing the matching FlatShelf and the
     * corresponding raw output value. If this butler was initialized with the
     * empty library, returns null.
     * 
     * @param b
     *            the book to examine
     */
    ...
    /**
     * Takes a double[] as input and returns a 2 element double[]. answer[0] =
     * maxValue
     * 
     * @param input
     * @return
