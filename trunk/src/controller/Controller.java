...
public class Controller {
    ...
    /**
     * Sorts a local shelf into a number of shelves based on similarities of
     * books
     * 
     * @param bookshelf
     *            the local bookshelf to split
     * @param num
     *            the max number of shelves to group it into
     * @return
     */
    ...
    /**
     * Returns an object with properties of the name of the shelf the book is
     * most likely to fit in
     * 
     * @param book
     * @return
     */
	...
	if (focus == null)
	    throw new IllegalArgumentException(
		    "Cannot add to a non Local shelf");
	focus.insert(book);
	...
	for (int id = 0; id < connections.size(); id++) {
		...
		// HB.addButler(new
		// RemoteLibraryButler(connections.get(id).getConnectionId(),cntrl,connections.get(id).getAlias()));
	    }
	}
    }
	...
	for (int id = 0; id < connections.size(); id++) {
		...
		if (conn.isConnected()) {
		    return importSelectBookshelves(conn.getImportedLibrary(),
			    conn.getLibrary(), shelves);
		} else
     ...
     * @param remote
     */
    private synchronized Library importAllBookshelves(Library local,
	    Library remote) throws NullPointerException {
	...
	if (focus == null)
	    throw new IllegalArgumentException(
		    "Cannot remove from a non Local shelf");
	if (!focus.contains(book))
	    throw new IllegalArgumentException(
		    "Shelf does not contain the book");
	focus.remove(book);
	...
	if (focus == null)
	    throw new IllegalArgumentException(
		    "Cannot remove a non Local shelf");
